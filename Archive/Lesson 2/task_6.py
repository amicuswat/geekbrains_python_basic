...
product_template = {'название': ("имя товара", str),
                    'цена': ('стоимость товара', int),
                    'количество': ('количество товара', int),
                    'ед': ('Единица измерения (шт, кг, и тд)', str),
                    }
...
for key in product_template:
    ...
    # добавляем в словать ключ из темплейта и список элементов который генерируем циклом
    # itm[1] в структуре списка продуктов это словарь и из него получаем значение по ключу
